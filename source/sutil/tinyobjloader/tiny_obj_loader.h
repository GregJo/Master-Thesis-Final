...
updateVertex(std::map<vertex_index, unsigned int> &vertexCache,
             std::vector<float> &positions, std::vector<int> &obj_ids, std::vector<float> &normals,
             std::vector<float> &texcoords,
             const std::vector<float> &in_positions,
             const std::vector<float> &in_normals,
             const std::vector<float> &in_texcoords, const vertex_index &i, const int obj_id) {
...
static bool exportFaceGroupToShape(
    ...
    const int material_id, const std::string &name, const int &obj_id, bool clearCache) {
  if (faceGroup.empty()) {
    return false;
  }
  ...
  for (size_t i = 0; i < faceGroup.size(); i++) {
    ...
    for (size_t k = 2; k < npolys; k++) {
      ...
      unsigned int v0 = updateVertex(
          vertexCache, shape.mesh.positions, shape.mesh.obj_ids, shape.mesh.normals,
          shape.mesh.texcoords, in_positions, in_normals, in_texcoords, i0, obj_id);
      unsigned int v1 = updateVertex(
          vertexCache, shape.mesh.positions, shape.mesh.obj_ids, shape.mesh.normals,
          shape.mesh.texcoords, in_positions, in_normals, in_texcoords, i1, obj_id);
      unsigned int v2 = updateVertex(
          vertexCache, shape.mesh.positions, shape.mesh.obj_ids, shape.mesh.normals,
          shape.mesh.texcoords, in_positions, in_normals, in_texcoords, i2, obj_id);
	  ...
	  shape.mesh.obj_ids.push_back(obj_id);
    }
  }
  ...
  while (inStream.peek() != -1) {
    ...
    if ((0 == strncmp(token, "usemtl", 6)) && isSpace((token[6]))) {
      ...
      bool ret = exportFaceGroupToShape(shape, vertexCache, v, vn, vt,
                                        faceGroup, material, name, obj_id, true);
      if (ret) {
